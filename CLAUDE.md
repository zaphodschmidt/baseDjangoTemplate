...
| Container | Docker Compose; `docker-compose.yml` + `.override.yml` locally |
...
backend/
  ...
  apps/
    README.md             the blessed-core list
    core/                 the nouns everything else hangs off
  conftest.py             shared fixtures
  schema.yml              generated by drf-spectacular; the contract
frontend/
...
**How it sets the standard here.** Run `make check` before every push. If you
add CI, it runs the same command — one definition of "verified", not two that
drift.
...
Two things this template does **not** ship, and both fail loudly rather than
guessing:

- **`docker-compose.prod.yml`.** Every remote-target script refuses to run until
  it exists. A deploy script that invents your production topology is worse than
  one that stops.
- **A configured `PROD_SSH`/`TEST_SSH`.** They are ssh *aliases* from
  `~/.ssh/config`, not hostnames, because that is where the key, user and port
  live — and because cron has no ssh-agent to fall back on.
...
Measured, not guessed. These are real and unfixed **in the template as shipped**,
and every one of them contradicts a rule above. Fix them in the first commits of
a new project — the smallest fix is named in each case.

| Where | Rule it breaks | Smallest fix |
|---|---|---|
| `backend/config/settings.py:24` | [Secrets](#7-security) 1 — a hardcoded `SECRET_KEY` is a secret in git | `os.environ['SECRET_KEY']`, with a `DEBUG`-only fallback |
| `backend/config/settings.py:27` | [Secrets](#7-security) 2 — `DEBUG = True` by default fails open | `DEBUG = os.getenv('DEBUG', '') == 'true'` |
| `backend/config/settings.py:29` | [Secrets](#7-security) 2 — `ALLOWED_HOSTS = ['*']` fails open | read from env; `['*']` only when `DEBUG` |
| `backend/config/settings.py:62` | [CORS](#7-security) 4 — `CORS_ALLOW_ALL_ORIGINS = True` | an explicit allow-list, or drop `django-cors-headers` entirely if one origin serves both |
| `backend/config/settings.py:46` | [Permissions](#7-security) 2 — no `DEFAULT_PERMISSION_CLASSES`, so DRF defaults to `AllowAny` and every new view is public | add `'DEFAULT_PERMISSION_CLASSES': ['rest_framework.permissions.IsAuthenticated']` |
| `backend/defaultApp/` | [Repo layout](#repo-layout) — the app is not under `apps/`, and there is no blessed-core list | move to `backend/apps/core/`, add `backend/apps/README.md` |
| no `backend/apps/README.md` | [Module boundaries](#1-module-boundaries) 1 — the blessed-core list is what makes rule 1 checkable | write it with the first app |
| no `docker-compose.prod.yml` | [The tools](#10-the-tools) — every remote target is unusable until it exists | write it, then set `COMPOSE_PROD` in `tools/project.env` |
| no CI | [Verification](#6-verification--the-gate) — `make check` locally is only as good as the discipline of whoever pushes | add a workflow that runs `make check`; until then, nothing stops an unverified push |
| no `pre-commit` config | [Git](#9-git) — the no-AI-trailer rule is unenforced | add `commit-msg` and formatting hooks |

The first five are one file and roughly twenty lines together. They are listed
separately because each fails open in a different direction, and fixing four of
five still leaves the app reachable.
